...
fs.writeFileSync(
  ...
  [
    ...
    "```",
    "",
    "굳혀 둔 표 둘이 핵심이다. **없으면 그 부분이 통째로 안 뜬다.**",
    "  `src/data/about.json`   개요 문장 1.4MB",
    "  `src/data/profile.json` 원형 두 개의 재료 0.8MB",
    "둘 다 서버에서만 읽는다.",
    "",
    ...
    "```",
    "",
    "`scripts/` 는 분기에 한 번 자료를 새로 만들 때만 필요하다.",
    "지금 당장은 `about.json` 과 `profile.json` 만 있으면 된다.",
    "",
    "## 안 건드린 것",
    "",
    ...
    "## 어디부터",
    "",
    "기업 개요와 원형 둘부터. 같은 카드라 한 덩어리로 붙이는 것이 맞다.",
    "**키가 필요 없어서 바로 된다** — `stock-brief` 가 바깥을 하나도 안 부르고",
    "굳혀 둔 표 셋만 읽는다. 원형에 쓸 외국인 보유비중만 `stock-detail` 에서",
    "오는데, 그것도 종목상세 카드가 이미 부르는 주소라 요청이 늘지 않는다.",
    "",
  ].join(줄),
);
