...
import { 매출표아님, 알짜조각, 기대배수, 밋밋하다, 눌러 } from "./매출표.mjs";
 ...
 * 된다 — 삼성전기가 매출 11.3조인데 22.6조로 잡혔다. 합계 줄이 있으면
 * 그것만 쓰고, 없을 때만 세부를 더한다.
 */
function 묶기(rows, 이름칸, 값칸) {
  const 소계칸 = (r) =>
...
function 구역에서(구역, 원, 배수, 손봄 = {}) {
  ...
  for (const 열림 of [...구역.matchAll(/<TABLE/gi)].map((x) => x.index)) {
    ...
    // 이름 칸 — 왼쪽부터 보되, 그 칸이 쓸모없을 때만 오른쪽으로 옮긴다.
    //
    // 매출실적표는 왼쪽일수록 뭉뚱그린 말이 온다.
    //
    //   매출 유형 | 품목 | 구분 | 제83기 반기 …
    //   제품      | 승용 | 내수 | 1,791,538
    //   제품      | RV   | 수출 | 16,584,421
    //
    // 알맹이 있는 이름이 둘은 나와야 사업부문 칸이다. 무조건 알맹이가 많은
    ...
    for (let c = 0; c < 값칸; c++) {
      const 있는것 = rows.filter((r) => (r[c] ?? "").trim());
      if (있는것.length < 2) continue;
      if (있는것.filter((r) => 숫자(r[c])).length / 있는것.length >= 0.5) continue;
      // 머리글(「매출 유형」)까지 알맹이로 센다. 빼는 편이 맞아 보이지만,
      // 빼면 한 칸 더 오른쪽으로 밀려 품목 상세를 집는다 — 시총 상위 100 중
    ...
    const 합 = 묶기(rows, 이름칸, 값칸);
    if (!합.size) { 버림("쓸 줄이 하나도 안 남음"); continue; }
