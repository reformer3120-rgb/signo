 ...
 */
const 정규 = (t) =>
  t.replace(/이차전지/g, "2차전지").replace(/[ㆍ·]/g, "").replace(/[/()]/g, " ").replace(/\s+/g, "");
...
for (const u of unassigned) {
  const d = 정규(u.desc);
  const scored = themesOfGroup(u.sheet)
    .map((t) => {
      let sc = 0;
      const why = [];
      for (const w of t.core ?? []) if (d.includes(정규(w))) { sc += 3; why.push(w); }
      for (const w of t.sub ?? []) if (d.includes(정규(w))) { sc += 1; why.push(w); }
      // also 는 짧은 문장용 낱말이다. 사업보고서 문어체가 아니라 사람이 쓰는 말로
      // 적어 둔 것이라 여기서만 쓴다 ("이동통신", "카지노", "정수기" 같은 것들).
      for (const w of t.also ?? []) if (d.includes(정규(w))) { sc += 3; why.push(w); }
      // 테마 이름이 문장에 그대로 있으면 가장 강한 단서다
      for (const w of nameWords(t.name)) if (d.includes(정규(w))) { sc += 2; why.push(w); }

      // 낱말이 통째로는 없어도 조각이 다 있으면 센다.
      // "자동차 부품" 을 찾는데 사람은 "자동차용 열관리시스템 … 부품사다" 라고 쓴다.
      // 붙어 있지 않을 뿐 둘 다 있으면 같은 뜻이다.
      for (const w of [...(t.core ?? []), t.name]) {
        const parts = w.split(/[\s·]+/).map((x) => 정규(x)).filter((x) => x.length >= 2);
        if (parts.length < 2) continue;
        if (parts.every((x) => d.includes(x)) && !d.includes(정규(w))) {
          sc += 2;
          why.push(w + "(조각)");
        }
      }
  ...
  if (scored.length) {
    ...
    for (const x of scored.slice(1)) if (x.sc >= scored[0].sc) 배정[u.code].add(x.t.name);
  } else {
    미배정.push(u);
  }
}
