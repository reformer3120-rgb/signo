...
const 거래처표 = /^주요?(거래처|고객|매출처)|고객사|^매출처|^고객[\dA-Z]|^Customer|^상위 ?\d+개사|^[A-Z]사$/i;
...
const 환율표 = /^(USD|EUR|JPY|CNY|CNH|GBP|AUD|CAD|CHF|VND|THB|IDR|INR|HKD|SGD|TWD|MXN|BRL|RUB|PLN|MYR|PHP|AED)[/／]/i;

const 금융표 =
  /은행$|증권$|캐피탈$|저축은행$|^미수금$|^보증금$|^매출채권$|^대여금$|^선급금$|^미수수익$|상각후원가|공정가치|현금및현금성|위험회피|^예금$|정기예금|보통예금|당좌예금|금전신탁|기타채권|손보$|화재$|해상$/;
// 채권 나이표와 지식재산권 표. 파서 쪽에서도 막지만, 이미 받아 둔 캐시에는
// 남아 있어 여기서 한 번 더 거른다.
//   NHN KCP    정상채권 · 발생일경과 후 1년 이상 · …
//   RF시스템즈  특허(등록) 83.3% · 특허(출원) 16.7%
const 채권표 = /채권$|발생일경과|연체|^정상$|회수의문|대손/;
const 지재권표 = /^특허(증|권|등록|출원)?$|^상표(등록증|등록|권|출원)?$|^디자인(등록)?(증|권)?$|^실용신안(증|권)?$|^PCT$/;
// 연결 상계 행
const 상계행 = /내부매출|연결조정|내부거래|상계제거|매출제거/;
// 표의 머리글이 조각으로 끼어든 것
//   테라뷰  「EOTPR 72.9% · TeraPulse 등 14.9% · TeraCota 12.1% · 제품명 0%」
const 머리글 = /^(제품명|품목|품명|구분|항목|사업부문|부문|종류|매출유형|유형|비고|매출현황|매출액현황|출유형|매출유형및품목|연결기준|별도기준|연결|별도|연도|회계연도|기간|년도)$/;
...
function 매출(code) {
  ...
  if (몇(자산표) >= 2 || 몇(금융표) >= 2 || 몇(채권표) >= 2 || 몇(지재권표) >= 2) return null;
  if (몇(환율표) >= 2) return null;
