...
export function looksTable(s) {
  ...
  if (nums > 8 && digits / s.length >= 0.15) return true;
  return 글로된표(s);
}
 ...
 * 숫자 없이 글자만으로 된 표.
 *
 * 위의 자릿수 잣대는 숫자가 있는 표만 잡는다. 사업부문·계열사·용도 표는
 * 이름만 늘어놓아 숫자가 거의 없다.
 *
 *   킵스파마   "사업부문 주요사업분야 주요제품 관련회사 비고 제약·바이오 의약품
 *              제조, 판매경구용 펩타이드 플랫폼 개발의약품 …"
 *              → 이 한 줄로 폐배터리·OLED·비만치료제가 한꺼번에 붙었다
 *   킵스파마   "[연의 용도] … 통신장비용 군수품원료 탄환제조 핵물질포장재 …"
 *              → 납의 쓰임새를 적은 표인데 방위산업·포장재로 잡혔다
 *   다산네트웍스 "[한국표준산업분류…] 사업부문 대상 회사 사업 내용 네트워크 외 …"
 *              → 계열사 표인데 의류·패션과 음식료로 잡혔다
 *
 * 표 머리글을 하나씩 적는 것은 끝이 없다. 대신 우리말의 성질을 쓴다 —
 * 진짜 문장에는 조사가 촘촘하고, 표는 이름만 나열해 조사가 거의 없다.
 *
 *   진짜  "연결회사는 … 사업부문으로 운영되고 있으며, … 개요는 다음과 같습니다."  0.94
 *   진짜  "광케이블 등 네트워크 인프라 구축 장비를 해외 시장에 수출하여 …"       0.70
 *   표    "사업부문 주요사업분야 주요제품 관련회사 비고 제약·바이오 …"            0.09
 *   표    "[한국표준산업분류 : 유선통신장비제조업 (26410)] 사업부문 대상 회사 …"  0.00
 *
 * 문턱은 0.4 로 뒀다. 0.4~0.55 를 들여다보니 진짜 문장이 나오기 시작한다
 * ("해상풍력 하부구조물 … 영위하는 전문 기업입니다" 가 0.46).
 * 짧은 줄은 밀도가 흔들려 60자부터만 본다.
 */
function 글로된표(s) {
  if (s.length < 60) return false;
  const 조사 = (s.match(/(는|은|이|가|을|를|의|에|로|와|과|도|며|고)\s/g) ?? []).length;
  return 조사 / (s.length / 10) < 0.4;
}
