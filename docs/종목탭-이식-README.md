...
2,497종목 중 2,435종목(97.5%)에 선다. 없는 종목(59)은 테마 편입 사유 한
줄로 대신하고, 그것도 없으면 주요사업 낱말만 뜬다(3종목). 카드가 통째로
비는 종목은 없다.

셋은 사업보고서 '사업의 개요' 에 자기 이야기가 아예 없는 회사다 — 업종
일반론(STX그린로지스), 회계 부문 구분 상투구(광명전기), 용어 해설표
(나라스페이스테크놀로지). 없는 문장을 지어내지 않는다.
...
**사업보고서를 12월 분기 자리에 그대로 넣으면 안 된다.** 그것은 연간이다.
삼성전자 2025년이면 938,374억이어야 할 자리에 3,336,059억이 뜬다 — 3.6배다.
...
누적은 `thstrm_add_amount` 쪽에 따로 있다. 삼성전자 2025년으로 확인한 값이다.
...
3분기 누적을 못 받으면 그 칸은 비운다. 연간을 분기인 척 내놓느니 낫다.
