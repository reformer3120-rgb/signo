...
export function CloseReportButton() {
  ...
  const load = async () => {
    ...
    setErr("");
    try {
      const r = await fetch("/api/close-report");
  ...
  const download = () => {
    ...
    a.href = url;
    a.download = `signo-마감리포트-${report.date}.txt`;
    a.click();
    ...
    <div className="rounded-xl border border-line bg-surface px-4 py-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="min-w-0 flex-1">
          <div className="text-sm font-semibold">장 마감 리포트</div>
          <div className="text-[11px] text-muted mt-0.5">
            지수·수급·등락·신고저·시총상위·섹터·국채금리 마감 데이터를 텍스트로 저장
            {report && !report.closed && " · 15:40 이전이라 장중 잠정치입니다"}
          </div>
        </div>
