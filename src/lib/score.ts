 ...
 *
 * ── 휩쏘 거르기 ──────────────────────────────────────────
 * 교차는 후행 지표라 횡보장에서 오신호가 잦다. 그래서 둘 다 만족해야 크로스로
 * 친다. 하나라도 어긋나면 크로스로 내보내지 않고 배열 상태만 낸다.
...
export function maRead(closes: number[], volumes?: number[]): MaRead {
  ...
  // 확인되지 않은 교차는 교차로 치지 않는다.
  //
  // 예전에는 미확인도 크로스로 내보내고 화면에 "골든크로스?" 라고 적었다.
  // 물음표가 붙은 신호는 읽는 사람에게 아무 말도 하지 않는다 — 사라고도
  // 사지 말라고도 못 한다. 게다가 점수는 0.75 로 정배열과 같아서, 걸러낸다고
  // 만든 장치가 사실상 아무것도 안 걸렀다.
  //
  // 지금은 60일선 방향과 거래량이 받쳐 줄 때만 크로스라고 부른다. 아니면
  // 그냥 배열 상태(정배열·역배열·혼조)로 돌아간다 — 그것이 지금 아는 전부다.
  const cross = fresh && confirmed
    ? { type: (up ? "골든크로스" : "데드크로스") as "골든크로스" | "데드크로스", days, confirmed, why }
    : null;

  // ── 점수. 크로스가 아니면 배열 상태로만 매긴다.
  const score = cross
    ? up ? 1 : 0
