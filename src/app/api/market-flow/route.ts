...
import { hasKIS, foreignInstitution, nxtActive, currentSession, type FiBy } from "@/lib/kis";
 ...
 */
async function flow(market: "ALL" | "KOSPI" | "KOSDAQ", dir: "buy" | "sell", by: FiBy) {
  const session = currentSession();
  const rows = await foreignInstitution(market, 15, dir, by).catch(() => []);
...
export async function GET(req: Request) {
  ...
  const b = searchParams.get("by");
  const by: FiBy = b === "foreign" || b === "inst" ? b : "net";
  try {
    // 세션이 바뀌면 이전 세션 캐시가 이월되지 않도록 키에 포함
    const session = currentSession();
    const r = await cached(`market-flow8:${session}:${market}:${dir}:${by}`, 60, () =>
      flow(market, dir, by),
    );
    return NextResponse.json({ market, session, dir, by, ...r });
  } catch (e) {
